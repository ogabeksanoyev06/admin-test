<template>
   <div class="flex flex-1 flex-col gap-4">
      <div class="flex items-center justify-between">
         ...
         <ModalTestScienceCreate :test_type="route.params.id" :classId="route.params.classId" @science-added="handleAddScience" />
      </div>
            ...
            <Table>
               ...
               <TableBody>
                  <TableRow v-for="(item, i) in sciences?.data?.sort((a, b) => a.name_uz[0].localeCompare(b.name_uz[0]))" :key="i" class="odd:bg-muted/50">
                     ...
                     <TableCell>
                        <NuxtLink :to="`/test-types/${route.params.id}/${route.params.type}/school-test/${route.params.classId}/${item._id}`" class="text-primary">
                           {{ item.name_uz }}
                        </NuxtLink>
                     </TableCell>
...
const { getClassNumberSciences } = scienceStore;
...
const { data: sciences, refresh } = await useAsyncData('sciences', async () => {
   const response = await getClassNumberSciences(route.params.classId);
   return response;
});
</script>
