<template>
   <div class="flex flex-1 flex-col gap-4">
      <div class="flex items-center justify-between">
         <h1 class="text-lg font-medium md:text-2xl">Milliy sertifikat uchun test yaratish bo'limi</h1>
      </div>
               ...
               <div class="flex flex-col items-start gap-4">
                  <div class="flex flex-col gap-2">
                     <VField name="questions" rules="required" v-model="text">
                        ...
                        <CommonEditor v-model="text" class="" />
      ...
      <div class="flex flex-col gap-4" v-if="mathView">
         ...
         <ul v-for="(question, i) in rawTests.questions" :key="i">
            <h3 class="text-base font-medium" v-katex:auto v-html="i + 1 + '. ' + question.question_text"></h3>
            <li v-for="option in question.options" class="p-2 border-b" :class="{ 'bg-secondary': option.is_correct }" v-katex:auto v-html="option.text" />
         </ul>
      </div>
   </div>
</template>
...
const { updateSubjectById, getSubjectById } = scienceStore;
...
const handleSubmitForm = async () => {
   ...
   try {
      const res = await getSubjectById(route.params.subjectId);
      if (res.status === 'success') {
         const existingQuestions = res.data.questions || [];
         const newQuestions = prepareQuestions(prepareMathView(text.value));
