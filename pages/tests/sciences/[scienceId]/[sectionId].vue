            ...
            <Table>
               ...
               <TableBody>
                  <TableRow v-for="(item, i) in subjects?.data" :key="i" class="odd:bg-muted/50">
                     ...
                     <TableCell class="font-medium"> {{ item?.name_uz }}</TableCell>
                     ...
                     <TableCell class="flex flex-wrap gap-2">
                        <ModalTestSubjectUpdate :subjectId="item._id" :sectionId="sectionId" @subject-updated="handleUpdateSubject" />
...
const { data: subjects, refresh } = await useAsyncData('subjects', async () => {
   return await getSubjectSectionById(sectionId);
});
</script>
