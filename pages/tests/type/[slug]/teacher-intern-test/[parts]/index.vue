            ...
            <Table>
               ...
               <TableBody>
                  <TableRow v-for="(item, i) in parts?.data" :key="i" class="odd:bg-muted/50">
                     ...
                     <TableCell class="font-medium">
                        <NuxtLink :to="`/tests/type/${route.params.slug}/teacher-intern-test/${route.params.parts}/${item._id}`" class="text-primary">
                           {{ item.name_uz }}
                        </NuxtLink>
                     </TableCell>
                     <TableCell>{{ $dayjs(item.createdAt).format('DD.MM.YYYY HH:mm:ss') }} </TableCell>
                     <TableCell class="flex flex-wrap gap-2">
                        <ModalTestUnitUpdate :scienceId="partsId" :sectionId="item._id" @section-updated="handleUpdateSection" />
...
const partsId = route.params.parts;
...
const { data: parts, refresh } = await useAsyncData('parts', async () => {
   return await getSectionByScienceId(route.params.parts);
});
</script>
