<template>
   <div class="flex flex-1 flex-col gap-4">
      <div class="flex items-center justify-between">
         ...
         <ModalTestScienceCreate :test_type="route.params.slug" @science-added="handleAddScience" />
      </div>
            ...
            <Table>
               ...
               <TableBody>
                  <TableRow v-for="(item, i) in sciences?.data" :key="i" class="odd:bg-muted/50">
                     ...
                     <TableCell>
                        <NuxtLink :to="`/tests/type/${route.params.slug}/teacher-intern-test/${item._id}`" class="text-primary">
                           {{ item.name_uz }}
                        </NuxtLink>
                     </TableCell>
...
const { data: sciences, refresh } = await useAsyncData('sciences', async () => {
   return await getScienceTestTypesById(route.params.slug);
});
</script>
