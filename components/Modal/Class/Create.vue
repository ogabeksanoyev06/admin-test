...
const { loading } = storeToRefs(schoolsStore);

const { createSchools } = schoolsStore;
...
const handleSubmitForm = async () => {
   try {
      ...
      if (response.status === 'success') {
         ...
         form.region = '';
         isOpen.value = false;
         showToast("Maktab muvaffaqiyatli qo'shildi!", 'success');
      } else {
   ...
   <div>
      <Dialog v-model:open="isOpen">
         ...
         <DialogContent class="p-6">
            <DialogHeader>
               <DialogTitle>Maktab yaratish </DialogTitle>
            </DialogHeader>
