...
const scienceById = async () => {
   try {
      const scienceId = await getScienceById(props.scienceId);
      if (scienceId.status === 'success') {
         form.name_uz = scienceId.data.name_uz;
         form.name_ru = scienceId.data.name_ru;
         form.name_en = scienceId.data.name_en;
      }
   } catch (error) {
      console.log(error);
   }
...
const handleSubmitForm = async () => {
   try {
      const response = await updateScienceById(props.scienceId, form);
      if (response.status === 'success') {
         emit('science-updated');
         form.name_en = '';
         form.name_ru = '';
         form.name_uz = '';
         isOpen.value = false;
   ...
   <div>
      <Dialog v-model:open="isOpen">
         ...
         <DialogContent class="p-6">
            <DialogHeader>
               <DialogTitle>Fan yaratish </DialogTitle>
            </DialogHeader>
            <VForm @submit="handleSubmitForm" v-slot="{ errors }">
               <div class="grid gap-4 mt-4">
                  <div class="grid gap-2">
                     <VField name="name_uz" rules="required" v-model="form.name_uz">
                        <Label for="schooltype">Fan nomi o'zbekcha</Label>
                        <Input v-model="form.name_uz" id="name_uz" type="text" placeholder="Fan uz" />
