...
const { deleteSubjectById } = scienceStore;
...
const confirmDelete = async () => {
   try {
      const response = await deleteSubjectById(props.subjectId);
      if (response?.status === 'success') {
         showToast("Mavzu muvaffaqiyatli o'chirildi!", 'success');
         emit('subject-deleted');
         isOpen.value = false;
      } else {
         showToast("Mavzu o'chirishda xatolik yuz berdi!", 'error');
      }
   } catch (error) {
      showToast("Mavzu o'chirishda xatolik yuz berdi!", 'error');
   }
      ...
      <Dialog v-model:open="isOpen">
         ...
         <DialogContent class="sm:max-w-[600px] p-4">
            <div class="pt-6 p-5 flex flex-col items-center">
               ...
               <p class="leading-130 text-gray-700 text-sm">Siz Mavzuni o'chirmoqchimisiz?</p>
