...
const { deleteTestTypeById } = testStore;
...
const confirmDelete = async () => {
   try {
      const response = await deleteTestTypeById(props.testTypeId);
      if (response?.status === 'success') {
         showToast("Test turi muvaffaqiyatli o'chirildi!", 'success');
         emit('testType-deleted');
         isOpen.value = false;
      } else {
         showToast("Test turi o'chirishda xatolik yuz berdi!", 'error');
      }
   } catch (error) {
      showToast("Test turi o'chirishda xatolik yuz berdi!", 'error');
   }
      ...
      <Dialog v-model:open="isOpen">
         ...
         <DialogContent class="sm:max-w-[600px] p-4">
            <div class="pt-6 p-5 flex flex-col items-center">
               ...
               <p class="leading-130 text-gray-700 text-sm">Siz bu test turini o'chirmoqchimisiz?</p>
