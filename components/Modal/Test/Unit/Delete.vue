...
const emit = defineEmits(['section-deleted']);
...
const { deleteSectionById } = scienceStore;
...
const confirmDelete = async () => {
   try {
      const response = await deleteSectionById(props.sectionId);
      if (response?.status === 'success') {
         showToast("Bo'lim muvaffaqiyatli o'chirildi!", 'success');
         emit('section-deleted');
         isOpen.value = false;
      } else {
         showToast("Bo'lim o'chirishda xatolik yuz berdi!", 'error');
      }
   } catch (error) {
      showToast("Bo'lim o'chirishda xatolik yuz berdi!", 'error');
   }
      ...
      <Dialog v-model:open="isOpen">
         ...
         <DialogContent class="sm:max-w-[600px] p-4">
            <div class="pt-6 p-5 flex flex-col items-center">
               ...
               <p class="leading-130 text-gray-700 text-sm">Siz Bo'limni o'chirmoqchimisiz?</p>
